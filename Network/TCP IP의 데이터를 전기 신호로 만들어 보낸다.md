...
1 - TCP 담당 부분은 데이터 송-수신 동작의 시작을 나타내는 제어정보를 기록한 TCP 헤더를 만든다(SYN이라는 컨트롤 비트 1로 만든다)
- TCP 헤더의 송신처와 수신처의 포트번호로 접속하는 소켓을 지정함
2 - TCP 헤더를 만든 이후, IP를 담당하는 곳에 건네 송신하도록 의뢰한다
...
다음과 같이 동작한다
- 접속 동작 개념
	- 1 - 클라이언트에서 서버에 보내는 시퀀스 번호의 초기 값을 산출해 서버에 통지한다
	- 2 - 서버는 초기값으로부터 ACK 번호, 시퀀스 번호를 산출하여 클라이언트에 통지한다
...
- IP 담당 부분은 패킷 운반 중 입구 부분
- 패킷을 운반하는 것은 허브나 라우터 같은 네트워크 기기의 역할
